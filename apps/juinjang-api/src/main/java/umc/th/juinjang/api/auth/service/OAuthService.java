...
public class OAuthService {
	...
	public LoginResponseDto kakaoLogin(Long targetId, KakaoLoginRequestDto kakaoReqDto) {
		...
		if (getMemberByEmail.isPresent() && getMemberByTargetId.isEmpty()) {
			if (!getMemberByEmail.get()
				.getProvider()
				.equals(MemberProvider.KAKAO)) {  // 이미 회원가입했지만 Kakao가 아닌 다른 소셜 로그인 사용
		...
		} else if (getMemberByEmail.isEmpty()
			&& getMemberByTargetId.isEmpty()) {    // 회원가입이 안되어있는 경우 -> 에러 발생. 회원가입 해야 함
	...
	public LoginResponseDto kakaoSignUp(Long targetId, KakaoSignUpRequestDto kakaoSignUpReqDto) {
		...
		if (getMember.isPresent() && getTargetId.isEmpty() && getMember.get()
			.getProvider()
			.equals(MemberProvider.APPLE)) {
			throw new MemberHandler(MEMBER_NOT_FOUND_IN_KAKAO);
	...
	public LoginResponseDto appleLogin(AppleLoginRequestDto appleLoginRequest) {
		...
		Member member = null;
		if (findSub.isEmpty() && findEmail.isPresent() && findEmail.get()
			.getProvider()
			.equals(MemberProvider.KAKAO)) {  // 이미 회원가입했지만 Apple 아닌 다른 소셜 로그인 사용
	...
	public LoginResponseDto appleSignUp(AppleSignUpRequestDto appleSignUpRequestDto) {
		...
		Member member = null;
		if (findSub.isPresent() && findEmail.isPresent() && (findSub.get().getMemberId() == findEmail.get()
			.getMemberId())
			&& findSub.get().getProvider().equals(MemberProvider.APPLE)) {  // 이미 회원가입한 회원인 경우 -> 에러 발생
			throw new MemberHandler(ALREADY_MEMBER);
		} else if (!findSub.isPresent() && findEmail.isPresent() && findEmail.get()
			.getProvider()
	...
	// 카카오 로그인 (회원가입 해야하는 경우)
	@Transactional
	public LoginResponseVersion2Dto kakaoSignUpVersion2(Long targetId,
		KakaoSignUpRequestVersion2Dto kakaoSignUpReqDto) {
		String email = kakaoSignUpReqDto.getEmail();
		log.info(kakaoSignUpReqDto.getEmail());
		...
		if (getMember.isPresent() && getTargetId.isEmpty() && getMember.get()
			...
			member = memberRepository.save(
				Member.createKakaoMember(
					...
					targetId,
					kakaoSignUpReqDto.getNickname(),
					kakaoSignUpReqDto.getAgreeVersion()
				)
			);
		}
